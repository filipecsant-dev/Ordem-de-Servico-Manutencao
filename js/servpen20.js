$(document).ready(function(){
	...
	setTimeout(function() {
	  listarpedidos2('ajax/controle.php', 'listar_pedidospendentes',true);
	}, 30000);

	listarpedidos('ajax/controle.php', 'listar_pedidospendentes');
	...
	//FECHAR
	$('.table').on("click", '#fechar_prod', function(){
		var cargo = $(this).attr('data-status');
		...
		$.ajax({
			...
			type: "POST",
			data: {acao: 'fechar_prod',  cargo: cargo},

			success: function(retorno){

				if(retorno === 'foi'){
					msgfun('Atualizado com sucesso!', 'sucesso');
					listarpedidos2('ajax/controle.php', 'listar_pedidospendentes',true);
				} else{
					msgfun('Erro ao atualizar!', 'erro');
				}
		...
		$.ajax({
			...
			success: function(retorno){
				...
				if(retorno === 'faltou'){
					...
					dados.fadeOut('slow', function(){
						listarpedidos('ajax/controle.php', 'listar_pedidospendentes', true);
	...
	//BOTAO AGUARD. PARADA
	$('.table').on("click","#aguard_parada", function(){
		var id = $(this).attr('data-id');
		var maquina = $(this).attr('data-maq') + " - " + $(this).attr('data-obs');


			$.post('ajax/caduser.php', {acao: 'aguard_parada', maquina: maquina},function(retorno){
				$('#ExemploModalCentralizado').modal({backdrop: 'static'});
				$('#TituloModalCentralizado').html('Aguardando Parada');
				conteudo.html(retorno);	
			});

			$('#ExemploModalCentralizado').on("submit", 'form[name="form_aguardparada"]', function(){
			var dados = $(this);
			var botao = dados.find(':button');
				...
				$.ajax({
					...
					success: function(retorno){
						if(retorno === "atualizou"){
							listarpedidos('ajax/controle.php', 'listar_pedidospendentes', true);
							dados.fadeOut('slow', function(){
								msgfun('SS '+id+' Aguardando parada de máquina', 'sucesso');
							});
						} else {
							msgfun('Erro ao alterar estado da SS!', 'alerta');
						}
					}
				});
		return false;
